...
<Flex fullWidth class="gap-3 items-center relative">
	...
	<Flex fullWidth class="pe-5 gap-2 relative items-center">
		<input 
			type="text" 
			class="bg-white w-full rounded-lg {navigator.searched && navigator.searchValue && navigator.searchValue.length && 'me-4'} text-black focus:outline-0 px-3 py-0.5"
			placeholder={LocalizeText('navigator.filter.input.placeholder')}
			bind:value={navigator.searchValue} 
			{onkeydown} 
		/>
		{#if navigator.searchValue && navigator.searchValue.length > 0}
			<Flex onclick={ () => { navigator.searchValue = ''; navigator.reloadCurrentSearch(''); } } class="absolute {navigator.searched && navigator.searchValue && navigator.searchValue.length ? 'right-11' : 'right-7'} cursor-pointer text-tertiary">
				<Fa icon={ faTimes } />
			</Flex>
		{:else}
	...
	{#if navigator.searched && navigator.searchValue && navigator.searchValue.length > 0}
