	...
	<Flex column class="w-18">
		...
		<Flex center column class="bg-default-primary border-r-2 border-solid border-default-hover shadow-[1px_0px_black] gap-2">
			<Flex id="hotel-view" center class="w-full h-7.5">
				{#if !getRoomSession()}
					<Button id="home-button" class="bg-(image:--toolbar-spritesheet) hover:-translate-0.5 w-8 h-7.5 bg-position-[-71px_-54px]" onclick={() => CreateLinkEvent("navigator/goto/home")} />
				{:else}
			...
			<Button id="avatar-image-button" center class="hover:-translate-y-0.5 hover:drop-shadow-[0px_2px_black] w-13 border-[3px] outline-black outline-2 rounded-full border-solid overflow-hidden border-default-hover bg-default-primary h-13 relative">
