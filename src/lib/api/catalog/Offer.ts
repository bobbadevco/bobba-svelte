import  {
	GetFurnitureData,
	GetProductDataForLocalization, type ICatalogPage, type IProduct, type IPurchasableOffer, LocalizeText, ProductTypeEnum
} from '$lib/api';
import { Product } from '$lib/api/catalog/Product';
...
export class Offer implements IPurchasableOffer
{
	...
	private _pricingModel: string | undefined;
	private _priceType: string | undefined;
	private readonly _offerId: number;
	private readonly _localizationId: string;
	private readonly _priceInCredits: number;
	private readonly _priceInActivityPoints: number;
	private readonly _activityPointType: number;
	private readonly _giftable: boolean;
	private readonly _isRentOffer: boolean;
	private _page: ICatalogPage | undefined;
	private readonly _clubLevel: number = 0;
	private readonly _products: IProduct[];
	private readonly _badgeCode: string | undefined;
	private readonly _bundlePurchaseAllowed: boolean = false;
	...
	{
		return this._page || {} as ICatalogPage;
	}
	...
	{
		if(!this._products || !this._products.length) return {} as IProduct;
	...
	public clone(): IPurchasableOffer
	{
		...
		for(const product of this._products)
		{
			...
			products.push(new Product(product.productType, product.productClassId, product.extraParam, product.productCount, productData || {} as IProductData, furnitureData || {} as IFurnitureData));
		}
